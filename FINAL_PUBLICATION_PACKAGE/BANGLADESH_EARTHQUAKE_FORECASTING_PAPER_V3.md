...
date: "FINAL_v1.0_FROZEN publication package — version 3 of the project paper"
version: "V3"
---

# About the Author

Ujayer Hasnat is a Data Scientist and Data Analyst specialising in probabilistic seismic hazard assessment and statistical seismology. In this study, I developed and validated a probabilistic earthquake forecasting system for Bangladesh, building on a merged USGS+ISC catalog of 5,779 events above the magnitude of completeness $M_c = 4.13$ and spanning the 1973–2024 instrumental period. My work focused on constructing a cell-wise Spatial Poisson baseline (FINAL_v1.0_FROZEN) on a 1°×1° grid, deriving per-cell Garwood exact Poisson confidence intervals, and retrospectively validating the model on nine chronological evaluation origins spanning 2015–2023. The baseline attains Brier 0.0242 (ECE 0.0087) at the 7-day $M \geq 4.5$ horizon and Brier 0.0763 (ECE 0.0322) at the 30-day horizon, with spatial holdout wins in all four quadrants.

Beyond the production baseline, I designed and evaluated three candidate extensions under a strictly leak-free dev/select/eval protocol: a Bayesian hierarchical Gamma–Poisson model with empirical-Bayes shrinkage (v2), an adaptive spatial smoother with Gaussian and Epanechnikov kernels (v3), and a region-specific ETAS formulation with depth-stratified fitting and depth-dependent spatial kernels (v4). None of the three delivers a statistically defensible improvement: v2 changes Brier by $\Delta \approx 0$ (verdict B, uncertainty improvement only); v3 fails the posterior predictive check and is REJECTED; v4 collapses to $K \approx 0$ in all four variants and is REJECTED. I attribute the persistent $R \approx 24\times$ Omori-style rate enhancement at sub-hour lags to short-lag catalog relocation artifacts rather than genuine tectonic aftershock cascades, a conclusion supported by the absence of magnitude scaling ($\alpha = 0$) in every ETAS variant. My ongoing research interests include prospective forecast validation under immutable hash-chained ledgers, statistical power analysis for small-sample seismological inference, and the integration of GCMT, BMD, and ISC-GEM historical catalogs to constrain the $M \geq 7$ rate.
...
See `BANGLADESH_EARTHQUAKE_FORECASTING_PAPER_V3.pdf` and `BANGLADESH_EARTHQUAKE_FORECASTING_PAPER_V3.tex` for the full typeset appendices.

---

*End of publication. Independent research by Ujayer Hasnat (Data Scientist; Data Analyst). The frozen CSVs, ledgers, and source code referenced throughout are available in the project repository under `FINAL_PUBLICATION_PACKAGE/`. The model code is FROZEN at version FINAL_v1.0_FROZEN; no further model development is planned.*
