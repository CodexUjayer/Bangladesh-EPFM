...
% V3: typesetting corrections and author branding
...
\fancyhead[L]{\footnotesize\itshape Ujayer Hasnat}
\fancyhead[R]{\footnotesize\itshape FINAL\_v1.0\_FROZEN}
\fancyfoot[C]{\thepage}
\fancyfoot[L]{\footnotesize v3 publication package}
...
{\large\bfseries Ujayer Hasnat}\par\vspace{0.3em}
{\small\itshape Data Scientist \,$\cdot$\, Data Analyst}\par\vspace{0.4em}
{\small\itshape FINAL\_v1.0\_FROZEN publication package --- version 3 of the project paper}\par\vspace{0.3em}
...
{\footnotesize\textsuperscript{$\ast$}\,\itshape Independent research by Ujayer Hasnat. The model code, frozen CSVs, and ledgers referenced throughout are available in the project repository.}\par\vspace{0.4em}
...
The empirical magnitude--frequency relation
\begin{equation}\label{eq:gr}
\log_{10} N = a - bM
\end{equation}
\cite{gutenberg1944} remains the most robust first-order description of earthquake occurrence. The $b$-value, typically near unity for tectonic seismicity, governs the relative frequency of small and large events and underwrites any magnitude-thresholded rate forecast. Estimation of $b$ from a finite catalog requires both a correct magnitude of completeness $M_c$ and a variance-stabilising estimator (typically the maximum-likelihood form of \cite{aki1965}). The Bangladesh catalog yields $b=0.808\pm0.010$ (Section~\ref{sec:gr}), a value consistent with the regional rate of intermediate-focus events.
...
\begin{tabular*}{\dimexpr\columnwidth+45pt\relax}{@{\extracolsep{\fill}}lcc}
\toprule
Metric & Value & Status \\
\midrule
...
The Gutenberg--Richter magnitude--frequency relation \eqref{eq:gr} \cite{gutenberg1944} is fitted to the merged catalog above $M_c=4.13$ using the maximum-likelihood estimator of \cite{aki1965},
...
\begin{equation}\label{eq:poisson-prob-generic}
...
\textbf{Brier score} (binary exceedance): for each $(i,t)$ forecast origin--cell pair,
\begin{equation}\label{eq:brier}
B = \frac{1}{N}\sum_{i,t}\bigl(p_{it} - y_{it}\bigr)^{2},
\end{equation}
where $p_{it}$ is the forecast probability and $y_{it}\in\{0,1\}$ is the observed outcome. The mean Brier over cells and origins is reported.

\textbf{Expected Calibration Error (ECE)}: the forecast probabilities are binned into seven equally-spaced bins $[0,1/7),[1/7,2/7),\dots$, and
\begin{equation}\label{eq:ece}
\mathrm{ECE} = \sum_{b=1}^{7}\frac{n_{b}}{N}\,\bigl|f_{b} - \bar{y}_{b}\bigr|,
\end{equation}
where $n_b$ is the bin count, $f_b$ the mean forecast in the bin, and $\bar y_b$ the observed event rate in the bin.

\textbf{Log score}: the mean negative binomial log-likelihood averaged over cells and origins is
\begin{equation}\label{eq:logscore}
\mathrm{LS} = -\frac{1}{N}\sum_{i,t}\bigl[y_{it}\log p_{it} + (1-y_{it})\log(1-p_{it})\bigr].
\end{equation}
...
\begin{equation}\label{eq:cell-rate}
...
\begin{equation}\label{eq:poisson-prob}
\begin{aligned}
\hat P_i(\Delta t) &= 1 - \exp\!\left(-\hat\lambda_i \, A_i \, \Delta t\right) \\
                  &= 1 - \exp\!\left(-N_i^{\text{train}} \, \Delta t / T_{\text{train}}\right).
\end{aligned}
\end{equation}
...
\begin{aligned}
\sum_{k=0}^{N_i} \frac{(\lambda^{\text{lo}} T)^k e^{-\lambda^{\text{lo}} T}}{k!} &= 0.975, \\
\sum_{k=N_i}^{\infty} \frac{(\lambda^{\text{hi}} T)^k e^{-\lambda^{\text{hi}} T}}{k!} &= 0.975,
\end{aligned}
\end{equation}
computed by bisection on the incomplete gamma function. The 95\% interval on the forecast probability is then
\begin{equation}\label{eq:garwood-prob}
\Bigl[\,1 - e^{-\lambda_i^{\text{hi}} \Delta t},\;\, 1 - e^{-\lambda_i^{\text{lo}} \Delta t}\,\Bigr].
\end{equation}
...
The Epidemic Type Aftershock Sequence (ETAS) model \cite{ogata1988} is the most widely used stochastic model for earthquake occurrence with explicit triggering. The standard ETAS conditional intensity is
\begin{align*}
\lambda(x,y,t \mid \mathcal{H}_t) = {}& \mu(x,y) \\
&{} + \sum_{j:\,t_j<t} K\, e^{\alpha(M_j - M_0)} \cdot \frac{(1+c)^p}{(t - t_j + c)^p} \\
&{} \quad \cdot \frac{1}{\pi\sigma^2}\, \exp\!\left(-\frac{\|\mathbf{x} - \mathbf{x}_j\|^2}{\sigma^2}\right),
\end{align*}
...
\subsection{The R$\approx$24$\times$ / K$\approx$0 contradiction}\label{sec:omori}
...
\begin{equation}\label{eq:bayes-gen}
...
\begin{equation}\label{eq:bayes-post-mean}
...
\begin{equation}\label{eq:eb-mll}
\begin{aligned}
\log p(\{N_i\} \mid a, b) = \sum_i \Bigg[ & \log \binom{N_i + a - 1}{N_i} + N_i \log\frac{T}{T + b} \\
& {} + a \log\frac{b}{T + b} \Bigg],
\end{aligned}
\end{equation}
...
\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular*}{\dimexpr\columnwidth+30pt\relax}{@{\extracolsep{\fill}}lcccc}
\toprule
...
\footnotesize
\resizebox{\textwidth}{!}{%
\begin{tabular*}{\dimexpr\textwidth+50pt\relax}{@{\extracolsep{\fill}}lcccccccc}
\toprule
...
\footnotesize
\resizebox{\columnwidth}{!}{%
\begin{tabular*}{\dimexpr\columnwidth+55pt\relax}{@{\extracolsep{\fill}}lccc}
\toprule
...
\begin{equation}\label{eq:garwood-chi2}
...
\begin{equation}\label{eq:gamma-poisson-post}
\lambda_i \mid N_i, T \sim \text{Gamma}\bigl(a + N_i,\; b + T\bigr),
\end{equation}
...
\begin{equation}\label{eq:eb-mll-appendix}
\begin{aligned}
\ell(a, b) = \sum_i \Big[ & \log\Gamma(N_i + a) - \log\Gamma(a) - \log(N_i!) \\
& {} + N_i \log T - (N_i + a)\log(T + b) + a \log b \Big].
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:kde-appendix}
...
\begin{align*}
\lambda(x, y, t \mid \mathcal{H}_t) = {}& \mu(x, y) \\
&{} + \sum_{j: t_j < t} K\, e^{\alpha(M_j - M_0)} \cdot g(t - t_j; c, p) \cdot f(\mathbf{x} - \mathbf{x}_j; \sigma),
\end{align*}
where the modified Omori kernel, normalised so that $\int_0^\infty g(\tau)\, d\tau = 1$, is
\begin{equation}\label{eq:omori-kernel}
g(\tau; c, p) = \frac{(c+p-1)^p}{(c+\tau)^p} \cdot \frac{1}{c},
\end{equation}
and the 2D Gaussian spatial kernel is $f(\mathbf{d}; \sigma) = (\pi \sigma^2)^{-1} \exp(-\|\mathbf{d}\|^2 / \sigma^2)$. The log-likelihood over the observation window $[0, T]$ is
\begin{equation}\label{eq:etas-loglik}
...
\begin{equation}\label{eq:bootstrap-deltaB}
\Delta B = \frac{1}{N_{\text{cells}}\, N_{\text{origins}}} \sum_{i, t} \bigl(B_{it}^{(2)} - B_{it}^{(1)}\bigr).
\end{equation}
...
The ISC Bulletin is accessed via the ISC web service (\texttt{www.isc.ac.uk/iscbulletin/\allowbreak search/\allowbreak fdsnws/\allowbreak event/\allowbreak 1/\allowbreak query}) with the same query parameters as the USGS download. The raw download contains 5{,}894 events. After removing events with null magnitudes, null depths, or depth greater than 800\,km, 5{,}576 events remain.
...
\text{Power}(\delta) = \Pr\!\left( \widehat{\Delta B} < -z_{1-\alpha} \cdot \text{SE} \mid \mu = \delta \right) = \Phi\!\left( \frac{\delta}{\text{SE}} + z_{1-\alpha} \right),
\end{equation}
...
\begin{equation}\label{eq:power-example}
...
{\small Independent research by Ujayer Hasnat (Data Scientist; Data Analyst). The frozen CSVs, ledgers, and source code referenced throughout are available in the project repository under \texttt{FINAL\_PUBLICATION\_PACKAGE/}. The model code is FROZEN at version FINAL\_v1.0\_FROZEN; no further model development is planned.}
